...
Page({
  ...
  // 首页课程更多
  moreshouye: function (event) {
    console.log(event)
    var that = this;
    wx.navigateTo({
      url: `/pages/electiveCourse/tuijian?searchType=${that.data.selTabIndex}`,
    })
  },
  // 最新课程——更多
  morekc: function () {
    ...
    util.getServerData({
      ...
      if (result.data) {
        for (var i = 0; i < result.data.length; i++) {
          result.data[i].progress = util.returnFloat(parseFloat(result.data[i].progress) * 100);
        }
      }
      // 请求数据为空
      // if (!result.length == 0) {
